...
public class RutinaDao extends AdapterDao<Rutina> {
    ...
    public Boolean checkSpecialCharacters() throws Exception {
        if(this.getRutina().getNombreRutina().matches(".*[!@#$%^&*()_+=\\[\\]{};':\"\\\\|,.<>\\/?].*")) {
            throw new Exception("El nombre de la rutina no puede contener caracteres especiales");
        }
        if(this.getRutina().getDescripcion().matches("*[!@#$^&*_+=\\[\\]{};'\"\\\\|<>\\/?]*")) {
            throw new Exception("La descripción de la rutina no puede contener caracteres especiales");
        }
        return true;
    }
