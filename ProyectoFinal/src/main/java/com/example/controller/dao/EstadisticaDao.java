...
public class EstadisticaDao extends AdapterDao<Estadistica> {
    ...
    public Boolean validations() throws Exception {
        if (!camposLlenos())
            return false;
        if (!checkBlanceSpaces())
            return false;
        if (!camposValidos())
            return false;
        if (!camposValidosMedidas())
            return false;
    ...
    public Boolean camposValidosMedidas() throws Exception {
        if (this.getEstadistica().getMedidaEspalda() > 70.0f) {
            throw new Exception("El campo medida espalda no puede ser mayor a 70 cm");
        }
        if (this.getEstadistica().getMedidaPierna() > 70.0f) {
            throw new Exception("El campo medida pierna no puede ser mayor a 70 cm");
        }
        if (this.getEstadistica().getMedidaBrazo() > 60.0f) {
            throw new Exception("El campo medida brazo no puede ser mayor a 60");
        }
        if (this.getEstadistica().getMedidaCintura() > 80.0f) {
            throw new Exception("El campo medida cintura no puede ser mayor a 80");
        }
        if (this.getEstadistica().getMedidaPecho() > 70.0f) {
            throw new Exception("El campo medida pecho no puede ser mayor a 70 cm");
        }
        if (this.getEstadistica().getPeso() > 250.0f) {
            throw new Exception("El campo peso no puede ser mayor a 250 kg");
        }
        if (this.getEstadistica().getAltura() > 220.0f) {
            throw new Exception("El campo altura no puede ser mayor a 220 cm");
        }
        return true;
    }
